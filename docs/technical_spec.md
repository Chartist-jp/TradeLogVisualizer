...
* **UI/UX仕様**:
  - ドラッグ＆ドロップで並び替え可能なチャートグループ。
  - チャート上の売買マーカー（Buy/Sell）には、取得額・売却額を通貨単位（¥/$）付きで精密に表示する。
  - **価格フォーマット**: 日本株は整数表示（四捨五入）、米国株は小数点第2位まで（第3位四捨五入）。
  - **ホバーUI**: 
    - クロスヘア連動の凡例（Legend）をチャート左上に表示（OHLCV, SMA各値）。
    - 横軸（日付軸）のツールチップから時刻表示を削除し、日付のみを表示。
  - 移動平均線（SMA）は線のみを表示し、価格スケールには個別の価格を表示させない。

### 4.3. ポートフォリオ分析機能 (Portfolio Analysis)
トレード全体の損益構造を可視化するヒストグラム機能。

#### 4.3.1. 2Dヒストグラム (損益率分布)
* **目的**: どの程度の利益率で利益/損失を出しているかの分布を把握する。
* **X軸**: 損益率 (%)。バケット（ビン）サイズは5%刻みで自動調整。
* **Y軸**: 損益額の合計 (通貨単位)。
* **色分け**: 利益バケット（X > 0）は緑、損失バケット（X < 0）は赤。
* **比較機能**: 
    - 2つの異なる期間（期間A vs 期間B）のヒストグラムを並列表示し、パフォーマンスの変化を分析可能にする。
...
#### 4.3.3. ライブラリ選定
* **Recharts**: 統計チャート（ヒストグラム、散布図）の描画に使用。Reactとの親和性が高く、カスタマイズが容易。